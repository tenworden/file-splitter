...
from tkinter import Tk, BOTH, NORMAL, DISABLED
    ...
    if os.path.isfile(file):
        ...
        if extension == ".xlsx":
            ...
            for i in range(no_of_file):
                # New files with index numbers
                file_path = file.replace(extension, "_{}{}".format(i, extension))
                file_data[i * chunk_size : (i + 1) * chunk_size].to_excel(
            ...
            for i in range(no_of_file):
                # New files with index numbers
                file_path = file.replace(extension, "_{}{}".format(i, extension))
                file_data[i * chunk_size : (i + 1) * chunk_size].to_csv(
                    file_path, index=False
                )
...
class Method(Frame):
    ...
    def base_method(self, entries):
        ...
        try:
            ...
            queue.put(message)
            exit()
        self.p1 = Process(target=splitter, args=(queue, file_path, chunk_size))
        self.p1.start()
        self.after(DELAY1, self.on_get_value)
